...
function ThisProductOnLocalStorage(event){
    ...
    let currentShoes = shoes.find(function(item){
        return item.sku == sku;
    })

    localStorage.setItem("this-product",JSON.stringify(currentShoes));
}
...
function onClickAddToCartButton(event) {
    ...
    SaveProductsinLocalStorageShoes(currentShoes)
    SaveNbrsProductLocalStorage()

    let end = document.querySelector(".end");
    end.classList.add("block");

    let miniCartItem = creatMiniCarteItem(currentShoes, quantity.value);
...
function SaveProductsinLocalStorageShoes(currentShoes){
    let ArrayLocalStorage = JSON.parse(localStorage.getItem("clothes"));
    
    if (ArrayLocalStorage) {
        ArrayLocalStorage.push(currentShoes);
...
function onClickAddToCartButtonHeart(event) {
    ...
    let currentBagInshoes = shoes.find(function (item) {
        return item.sku == sku
    })

    SaveProductsinHeartLocalStorage(currentBagInshoes)

    let heartFav = document.querySelector(".heartFav");
    heartFav.classList.add("block");

    let miniCartItem = creatMiniCarteItemFavHeart(currentBagInshoes,quantity.value);
...
for (let i = 0; i < titles.length; i++) {
    ...
    function SaveProductsinHeartLocalStorage(currentBagInshoes){
        let ArrayLocalStorage = JSON.parse(localStorage.getItem("clothes-Heart"));
        
        if (ArrayLocalStorage) {
            ArrayLocalStorage.push(currentBagInshoes);
...
function clickerlink(event) {
    ...
    let filterShoesArray = shoes.filter(item => item.Material == element.innerHTML || item.color == element.innerHTML || item.Date == element.innerHTML);

    product.innerHTML = " ";

    filterShoesArray.forEach(function (item) {
        product.prepend(createProductElement(item));
    });

    let numberItems = document.querySelector("#numberItems");

    numberItems.innerHTML = `<p>There is ${filterShoesArray.length} items founds</p>`;
}
...
function search() {
    ...
    let filterShoesArray = shoes.filter(item => item.color == input.value);
